// FILE: src/components/lib/editor-utils.ts
...
export function buildExportJson(items: Array<{ body: string; answer: AnswerJson; tags?: string[]; thumbnails?: string[] }>) {
...
export function normalizeImportPayload(
    ...
    for (const it of arr) {
        const o = it as { body?: unknown; answer?: unknown; tags?: unknown; thumbnails?: unknown };
        ...
        out.push({ body, answer, tags, thumbnails });
    }
...
function stableStringify(v: unknown) {
    // AnswerJson is shallow and stable, but keep deterministic ordering anyway
    return JSON.stringify(v, Object.keys(v as object).sort());
}

function hashString(s: string) {
    // deterministic, fast non-crypto hash (FNV-1a 32bit)
    let h = 0x811c9dc5;
